...
```{r}
new <- aiRactivation(data = train[,-3], aiRnet = loss.aiR$aiRnet.last, range.size = 10000)
...
```{r}
classify_obj <- aiRclassify(data = new$data_model, factor = levels(train$group), aiRnet = loss.aiR$aiRnet.last)
```
...
```{r}
aiRrate(data = train, factor = "group", aiRnet = loss.aiR$aiRnet.last)
```
