...
additional.c <- apply(apply(loss$loss.prop,2,function(x){x^2}),2,mean)/((apply(loss$loss.prop,2,mean))^2)  # The proportion mean of squares to square of mean... (iteration of each row vs average each row)
...
aiRrowdelta2 <- function(loss.prop,
  ...
  for(j in n:1) { #use back propigation... record desired changes for each input to each node... record in $change.w $change.b
    ...
    #row.work <- sqrt(length(new.loss))*new.loss*(abs(new.loss)/sum(abs(new.loss)))*(total.loss/((sum(apply(loss.prop,1,sum)*abs(row.loss)))))*(1/(g))
    additional.c <- ((apply(loss.prop,2,mean)^2))*(sum(abs(apply(loss.prop^2,1,sum)))/(g^3*(sum(apply(loss$loss.prop^2,1,sum)*abs(apply(loss$loss.prop^2,1,sum)))))) 
    #(sum(w^2)/(g^2))*(total.loss/(sum(apply(loss.prop,1,sum)*abs(apply(loss.prop,1,sum)))))
