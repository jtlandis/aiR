...
additional.c <- apply(loss$loss.prop^2,2,mean)/((apply(loss$loss.prop,2,mean))^2)  # The proportion mean of squares to square of mean... (iteration of each row vs average each row)
...
aiRrowdelta2 <- function(loss.prop,
  ...
  row.work <- sqrt(length(row.work))*row.work*(abs(1/(total.loss)))
  for(j in n:1) { #use back propigation... record desired changes for each input to each node... record in $change.w $change.b
    ...
    row.work <- sqrt(length(new.loss))*new.loss*(1/sum(abs(new.loss)))
  }
  return(aiRnet)
}
```
