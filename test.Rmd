...
system.time(aiRnet2 <- last_attempt())
...
# aiRnet2[[t]]$change.b <- abs(aiRnet2[[t]]$bias)*.row.work
...
#   aiRnet2[[a]]$change.b <- aiRnet2[[a]]$change.b/mult.w
# }

for(i in t:1) {
  x<- (aiRnet1[[i]]$change.w/aiRnet2[[i]]$change.w)[1,]
  print(x)
  print(mean(x))
  print((aiRnet1[[i]]$change.b/aiRnet2[[i]]$change.b))
}
...
```{r}
test4 <- function() {
a <- foreach(i=1:(length(loss$row.loss))) %dopar% {
  aiRrowdelta3(loss.prop = loss$loss.prop[i,],total.loss = loss$total.loss, aiRnet = aiRnet2,n = t)
}
test <- get_change_average(apply.test = a, n = length(aiRnet2))
...
```{r}
aiRnet3 <- aiRrowdelta1(loss.prop = apply(loss$loss.prop,2,mean), total.loss = loss$total.loss, aiRnet = newaiR, n = t)

for(i in t:1) {
   print((aiRnet1[[i]]$change.w/aiRnet3[[i]]$change.w)[1,])
}
```
...
```
```{r}
first.row.work <- function(loss.prop, prop.factor) {
  row.work <- sqrt(length(loss.prop))*loss.prop*(prop.factor)
  return(row.work)
}
w.change <- function(row.work,
                     aiRnet_layer) {
    w <- mat.opperation(x = abs(aiRnet_layer$weights), y = row.work, opperation = "*")
    b <- abs(aiRnet_layer$bias)*row.work
    vec.work <- list(w,b)
    names(vec.work) <- c("change.w","change.b")
    return(vec.work)
}

new.row.work <- function(row.work, aiRnet2_layer) {
  w <- sweep(abs(aiRnet2_layer$weights),2, row.work, "*")
  new.loss <- apply(w,1,mean) #consider taking this line out... replace overall algorithm to scale to returned sum(abs(row.work))... total data, not just each line.
  return(new.loss)
}
```
```{r}
last_attempt <- function(loss, aiRnet2, mc.cores) {
  .list <- vector("list",length(loss$row.loss))
  for(i in 1:length(.list)) {.list[[i]] <- loss$loss.prop[i,]}
  my.list <- mclapply(.list, first.row.work, total.loss = loss$total.loss, mc.cores = mc.cores)
  for(i in t:1) {
   my.list <- mclapply(my.list, w.change, aiRnet_layer = aiRnet2[[i]], mc.cores = mc.cores)
   change.w.list <- lapply(my.list, get_layer_change, layer = "change.w")
   aiRnet2[[i]]$change.w <- average_list(change.w.list)
   aiRnet2[[i]]$change.b <- average_list(lapply(my.list, get_layer_change, layer = "change.b"))
   if(i!=1) {my.list <- lapply(change.w.list, new.row.work)}
  } 
  return(aiRnet2)
}
```
...
```{r}
get_change.w <- function(loss.prop, row.work, aiRnet2_layer) {
  additional.c <- apply(loss.prop*abs(loss.prop),2,mean)/((apply(loss.prop,2,mean)*abs(apply(loss.prop,2,mean))))
  row.work <- additional.c*row.work
  aiRnet2_layer$change.w <- sweep(abs(aiRnet2_layer$weights), 2, row.work, FUN = "*")
  aiRnet2_layer$change.b <- abs(aiRnet2_layer$bias)*row.work
  return(aiRnet2_layer)
}

last_attempt <- function() {
  new.loss.prop <- loss$loss.prop
  prop.factor <- abs(apply(new.loss.prop,2,mean)/(total.loss))
for(i in t:1) {
  row.work <- first.row.work(loss.prop = apply(new.loss.prop,2,mean), prop.factor = prop.factor)
  aiRnet2[[i]] <- get_change.w(loss.prop = new.loss.prop, row.work = row.work, aiRnet2_layer = aiRnet2[[i]])
  new.loss.prop <- t(apply(new.loss.prop,1,new.row.work, aiRnet2_layer = aiRnet2[[i]]))
  prop.factor <- new.loss.prop/(apply(X = abs(new.loss.prop),MARGIN = 1, sum))
}
...
```{r}
apply_average <- function(x,aiRlayer, w.b, mc.cores) {
  a <- average_list(mclapply(mclapply(x,get_layer_change, layer = aiRlayer, mc.cores = mc.cores), get_layer_change, layer = w.b, mc.cores = mc.cores))
  return(a)
}

```
```{r}
get_change_average <- function(apply.test, n, mc.cores) {
  list.work <- vector("list",n)
  for(i in 1:n) {
    new.w <- apply_average(x = apply.test, aiRlayer = i, w.b = 1, mc.cores = mc.cores)
    new.b <- apply_average(x = apply.test, aiRlayer = i, w.b = 2, mc.cores = mc.cores)
...
```{r}
test2 <- function() {
  .list <- vector("list",nrow(loss$loss.prop))
  for(i in 1:length(.list)) { .list[[i]] <- loss$loss.prop[i,]}
  apply.test <- mclapply(X = .list,FUN = aiRrowdelta3, total.loss = loss$total.loss, aiRnet = aiRnet2, n = t)
...
desperate <- function() {
  browser()
  lapply(X = .list,FUN = aiRrowdelta3, total.loss = loss$total.loss, aiRnet = aiRnet2, n = t)
}
```

```{r}
system.time(apply.test <- test1())
for(i in 2:30) {
  print(i)
  print(system.time(apply.test <- last_attempt(loss = loss, aiRnet2 = aiRnet2, mc.cores = i)))
}
```
