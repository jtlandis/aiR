...
```{r}
new <- aiRactivation(data = train[,-3], layers = loss.aiR$layers, range.size = 10000)
```
...
```{r}
space.var <- aiRdevelop(data = train,x=x,y=y, var.classify = "group", layers = mylayers, cycles = 3, sample.size = .5, batch.size = 60, steps = 22)
