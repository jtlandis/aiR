...
#                         z = c(.1,.2,.4,.5,.7,-.8)))
#train <- train[,-4]
loss.aiR <- aiRrun(data = train,var.classify = "group",aiRnet = mylayers, sample.size = 1,cycles = 1000,batch.size = 50, method.propigate = "fast")


loss.aiR <- aiRrun(data = train, var.classify = "group", aiRnet = loss.aiR$aiRnet, sample.size = 1,cycles = 1000,batch.size = 300, method.propigate = "fast")
...
ggplot(data = loss.aiR$loss,aes(x = iteration)) +
  ...
  labs(list(y = "loss"))+
  coord_cartesian(ylim = c(min(loss.aiR$loss[!loss.aiR$loss==0])/100,max(loss.aiR$loss)))+
...
system.time(fast <- aiRrun(data = try.test, var.classify = "group",aiRnet = aiRnet1,cycles = 50,sample.size = 1,batch.size = 100,method.propigate = "fast"))
