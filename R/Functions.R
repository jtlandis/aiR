  ...
  for(k in 1:cycles) {
    ...
    for(i in 1:length(train.loss$row.loss)) { #for each input, start with loss
      aiRnet <- aiRrowdelta(loss.prop = train.loss$loss.prop[i,],
                            ...
                            total.loss = train.loss$total.loss[i],
                            aiRnet = aiRnet,
                            n = n)
    }
    last.loss <- aiRnet
...
aiRrowdelta <- function(loss.prop,
  ...
  for(j in n:1) { #use back propigation... record desired changes for each input to each node... record in $change.w $change.b
    ...
    row.work <- sqrt(length(new.loss))*new.loss*(abs(new.loss)/sum(abs(new.loss)))*(row.loss/total.loss)
  }
  return(aiRnet)
}
...
aiRrate <- function(data, factor, aiRnet, report.class = FALSE) {
  ...
  data.save <- data
  data <- data[,-index]
  classify <- aiRclassify(data = data, factor = levels(data.save[,index]), aiRnet = aiRnet)
